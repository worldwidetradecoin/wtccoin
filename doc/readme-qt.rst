WtcCoin-qt: Qt4 GUI for WtcCoin
===============================
...
Alternatively, install Qt Creator and open the `WtcCoin-qt.pro` file.

An executable named `WtcCoin-qt` will be built.
...
A warning for people using the *static binary* version of WtcCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of WtcCoin is linked against libdb5.3.
...
installed causes WtcCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
