...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #WtcCoinTEST2\r");
            Send(hSocket, "WHO #WtcCoinTEST2\r");
        } else {
            // randomly join #WtcCoin00-#WtcCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #WtcCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #WtcCoin%02d\r", channel_number).c_str());
        }
